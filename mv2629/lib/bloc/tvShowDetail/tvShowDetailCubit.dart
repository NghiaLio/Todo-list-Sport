...
class TvShowDetailCubit extends Cubit<TvShowDetailState> {
  final TvShowDetail repo;

  TvShowDetailCubit({required this.repo}) : super(TvShowDetailInitial());
  ...
  Future<void> getTvShowDetail(int id) async {
    ...
    try {
      ...
      final videoTrailer = await repo.getTvShowVideoTrailer(id);

      // 2. Get TV Detail
      var tvDetail = await repo.getTvShowDetail(id);
      if (tvDetail == null) {
      ...
      final similar = await repo.getSimilarTvShows(id, _page);
      if (similar != null) {
  ...
  Future<void> loadMoreSimilar(int id) async {
    ...
    if (currentState is! TvShowDetailLoaded || currentState.hasReachedMax) return;
